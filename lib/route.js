...
/**
 * [route - Routes are objects that are created in order to contain information required to configure the express router.
 		These route objects are intended to be consumes by the route-loader utility in order to register them with a
 		specified router reference.

 		Routes will track this information:
 		@middleware { Function OR Array of Functions } -
 ...
 */
var _allowed_verbs = ['all', 'delete', 'get', 'post', 'put', 'use'];
...
var Route = function Route(options) {
	...
	this.template_data = options.template_data = options.template_data || {};
	this.template_path = options.template_path; // left undefined for error catching if no value is passed in

	this.verb = options.verb = options.verb || 'get'; // defaults to get otherwise uses verb passed in
